   ...
   AFRAME.registerComponent('palm-tracking', {
       init: function () {
           ...
           function setupHand(handIndex, coordinates) {
               ...
               if (coordinates)
                   setCoordinates(marks, coordinates);
               else
                   resetHand(marks);
           }
           ...
           function computeHandRotation(boxs) {
               ...
               let normal = new THREE.Vector3().crossVectors(iwVector, pwVector).normalize();
           ...
           function onResults(results) {
               ...
               if (results.multiHandLandmarks[0]) {
                   if (results.multiHandedness[0].label == "Left") {
                       ...
                       document.querySelector("#myModel").setAttribute("rotation",
                           `${rotation.x.toFixed(2)} ${rotation.y.toFixed(2)} ${rotation.z.toFixed(2)}`)// Update the positions of the hand boxes based on the landmark data
                   ...
                   if(!document.querySelector("#myModel").getAttribute("gltf-model")){
                       ...
                       if(document.querySelector("#myModel").getAttribute("rotation").x < -80){
                           document.querySelector("#myModel").setAttribute("src", "3d/ring.png");
                       } else if (document.querySelector("#myModel").getAttribute("rotation").x > -100){
                       ...
                       if (results.multiHandLandmarks[0]) {
                           ...
                           // Change finger
                           let x = results.multiHandLandmarks[0][14].x - 0.5;
                           let y = -results.multiHandLandmarks[0][14].y + 0.5;
                           let z = results.multiHandLandmarks[0][14].z - 0.5;
                           document.querySelector("#myModel").setAttribute("position", `${x} ${y - 0.055} ${z}`);
